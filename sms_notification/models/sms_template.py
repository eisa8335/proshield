...
class SmsTemplate(models.Model):
    ...
    condition = fields.Selection([('order_placed', 'Order Place'),
                                  ('order_confirm', 'Order Confirm'),
                                  ('order_delivered', 'Order Delivered'),
                                  ('invoice_vaildate', 'Invoice Validate'),
                                  ('invoice_paid', 'Invoice Paid')], string="Conditions", help="Condition on which the template has been applied.")
    ...
    def onchange_condition(self):
        if self.condition:
            if self.condition in ['order_placed', 'order_confirm']:
    ...
    def get_body_data(self, obj):
        self.ensure_one()
        if obj:
            body_msg = self.env["mail.template"].sudo().render_template(
